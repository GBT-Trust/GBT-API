...
exports.registerUser = catchAsyncErrors(async (req, res) => {
  ...
  if (!user) {
    ...
    throw new ApiError(
      500,
      "Something went wrong while registering the user! Maybe an Internet Connection issue"
    );
  }
  ...
  return res
    .status(201)
    .json(
      new ApiResponse(200, { createdUser, referralCode }, "User registered")
    );
});
...
exports.updateImages = catchAsyncErrors(async (req, res) => {
  ...
  if (avatar) {
    const uploadedAvatar = await updateOnCloudinary(
      avatar?.path,
      req?.user?.avatar
    );
  ...
  if (aadhar) {
    const uploadedAadhar = await updateOnCloudinary(
      aadhar?.path,
      req?.user?.aadharCard
    );
...
exports.loginUser = catchAsyncErrors(async (req, res) => {
  ...
  if (!email || !password) {
    throw new ApiError(
      400,
      "phone number or username and password is required is required"
    );
  }
...
exports.forgotPassword = catchAsyncErrors(async (req, res) => {
  ...
  const resetUrl = `${req.protocol}://gbt-trust-xi.vercel.app/reset/${resetPasswordToken}`;
...
exports.getTeamMembers = catchAsyncErrors(async (req, res) => {
  ...
  res
    .status(200)
    .json(new ApiResponse(200, { success: true, teams: teams.refers }));
});
...
exports.risingStars = catchAsyncErrors(async (req, res) => {
  ...
  return res
    .status(200)
    .json(new ApiResponse(200, users, "Top 10 Rising Stars"));
});

exports.sendMail = catchAsyncErrors(async (req, res) => {
  const mailOptions = {
    from: process.NODEMAILER_EMAIL,
    to: req.body.email,
  ...
  return res
    .status(200)
    .json(new ApiResponse(200, information, "Email sent successfully"));
});
...
exports.getAllUserBelow = catchAsyncErrors(async (req, res) => {
  ...
  if (
    currUser.level.toString() !== level.toString() &&
    currUser &&
    currUser.role === "user"
  ) {
  ...
  return res
    .status(200)
    .json(new ApiResponse(200, users, "Referred Users Details"));
});
...
exports.newGroup = catchAsyncErrors(async (req, res) => {
  ...
  const users = await Promise.all(
    groupMembers.map(async (member) => {
      const user = await User.findOne({
        $or: [{ email: member }, { contact: member }],
      });
      if (!user) {
        throw new ApiError(
          404,
          `User with email or contact ${member} does not exist`
        );
      }
      return user._id;
    })
    ...
    return res
      ...
      .json(
        new ApiResponse(
          200,
          ePins,
          `Successfully generated ${numberOfPins} ePIN(s) for Member ID ${memberId}`
        )
      );
  } catch (error) {
...
exports.generateUserTree = catchAsyncErrors(async (req, res) => {
  ...
  res
    .status(200)
    .json(new ApiResponse(200, tree, "Tree Generated Successfully"));
});
...
exports.activeUsers = catchAsyncErrors(async (req, res) => {
  ...
  res
    .status(200)
    .json(new ApiResponse(200, users, "Active Users Found Successfully"));
});
...
exports.verifyUser = catchAsyncErrors(async (req, res) => {
  ...
  res
    ...
    .json(
      new ApiResponse(
        200,
        { referralCode: savedUser.track.code, userid: savedUser._id },
        status === true ? "user approved" : "user not approved"
      )
    );
});
...
exports.referralLinkGenerate = catchAsyncErrors(async (req, res) => {
  ...
  try {
    ...
    req.user.referralUrl = `${getBaseUrl(
      req
    )}/referral/generated-link/:referralCode=${referralCode}`;
    await req.user.save();
...
exports.updateTrack = catchAsyncErrors(async (req, res) => {
  ...
  return res
    .status(200)
    .json(new ApiResponse(200, owner, "Referral link accessed successfully"));
});
  ...
  return res
    ...
    .json(
      new ApiResponse(
        200,
        referralAmount,
        "Referral link accessed successfully"
      )
    );
});
...
exports.calculateLevel = catchAsyncErrors(async (req, res) => {
  ...
  return res
    .status(200)
    .json(
      new ApiResponse(200, leveldata, "Referral link accessed successfully")
    );
});
